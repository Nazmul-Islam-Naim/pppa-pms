    ...
    <div class="row gutters">
      ...
      <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">
        @if(!empty($single_data))
          {!! Form::open(array('route' =>['add-capital-cost.update', $single_data->id],'method'=>'PUT','files'=>true)) !!}
          <?php $info ="Update";?>
        @else
        {!! Form::open(array('route' =>['add-capital-cost.store'],'method'=>'POST','files'=>true,'enctype'=>'multipart/form-data')) !!}
          <?php $info ="Add";?>
        @endif
        <div class="card">
          <div class="card-header">
            <div class="card-title">{{$info}} Capital Cost To Project</div>
          </div>
          <div class="card-body">
            <!-- Row start -->
            <div class="row gutters">
              <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                ...
                <div class="field-wrapper">
                  <div class="input-group">
                    <select name="cost_id" class="form-control select-single js-states" data-live-search="true" requried="" autocomplete="off">
                      <option value="">Select</option>
                      @foreach( $allcost as $cost)
                      <option value="{{$cost->id}}" {{(!empty($single_data) && $cost->id == $single_data->cost_id) ? 'selected':''}}>{{$cost->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="field-placeholder">Capital Cost<span class="text-danger">*</span></div>
                </div>
                <!-- Field wrapper end -->
                <!-----------------doc -------------------------->
                <!-- Field wrapper start -->
      ...
      <div class="col-xl-8 col-lg-8 col-md-8 col-sm-12 col-12">
        <div class="card card-primary">
          <div class="card-header d-flex justify-content-between align-items-center">
              <h3 class="card-title">Capital Cost To Project List</h3>
            </div>
          <!-- /.box-header -->
          <div class="card-body">
            <div class="row">
   ...
   function dateFormat(data) { 
	...
	$(document).ready(function() {
      ...
      var table = $('#example').DataTable({
			...
			processing: true,
      deferRender : true,
			ajax: "{{route('add-capital-cost.index')}}",
...
$("#example").on("click", ".button-delete", function(e) {
			...
			var id = $(this).data('id');
			var url = '{{route("add-capital-cost.destroy",":id")}}';
